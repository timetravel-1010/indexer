...
func TestParseFullBody(t *testing.T) {
	...
	bodyString, ok := (*em)["Body"].(string)
	if !ok {
		t.Fatalf("Error casting body field into string type")
	}

	if strings.Compare(bodyString, ex) != 0 {
		t.Fatalf("got %q, expected %s", bodyString, ex)
	}
}
...
func Test26(t *testing.T) {
	...
	exFrom := "phillip.allen@enron.com"
	exTo := []*mail.Address{{Address: "john.lavorato@enron.com"}}
	//	exBCC := []*mail.Address{}
	//	exSubject := "Re: This morning's Commission meeting delayed"
	//	exMimeVersion := "1.0"
	//exContentType := "text/plain; charset=us-ascii"
	//exContentTransferEncoding := "7bit"
	//exXFrom := "Phillip K Allen"
	//exXTo := []*mail.Address{&mail.Address{Address: "John J Lavorato <John J Lavorato/Corp/Enron@ECT>"}}
	//exXcc := []*mail.Address{}
	//exXbcc := []*mail.Address{}
	//exXFolder := `\Phillip_Allen_Jan2002_1\Allen, Phillip K.\'Sent Mail`
	//exXOrigin := "Allen-P"
	//exXFileName := "pallen (Non-Privileged).pst"
	exBody := `
...
<Embedded StdOleLink>`
	...
	em, ok := e.(*StdEmail)
	if !ok {
		t.Fatalf("Error casting EmailI into StdEmail struct")
	}

	MessageID, ok := (*em)["Message-Id"].(string)
	if !ok {
		t.Fatalf("Error casting MessageId to string")
	}

	if strings.Compare(MessageID, exMessageID) != 0 {
		t.Fatalf("got %s, expected %s.", MessageID, exMessageID)
	}

	Date, ok := (*em)["Date"].(string)
	if !ok {
		t.Fatalf("error casting Date to string")
	}
	if strings.Compare(Date, exDate) != 0 {
		t.Fatalf("got %s, expected %s.", Date, exDate)
	}

	From, ok := (*em)["From"].(string)
	if !ok {
		t.Fatalf("error casting From to string")
	}
	if strings.Compare(From, exFrom) != 0 {
		t.Fatalf("got %s, expected %s.", From, exFrom)
	}

	To, ok := (*em)["To"].([]*mail.Address)
	if !ok {
		t.Fatalf("error casting To to string")
	}
	if !testEq(To, exTo) {
		t.Fatalf("got %v, expected %v.", To, exTo)
	}

	Body, ok := (*em)["Body"].(string)
	if !ok {
		t.Fatalf("error casting body to string")
	}
	if strings.Compare(Body, exBody) != 0 {
		t.Fatalf("got %s, expected %s.", Body, exBody)
	}

}

func testEq(a, b []*mail.Address) bool {
	if len(a) != len(b) {
		return false
	}
	for i := range a {
...
func TestSubject(t *testing.T) {
	...
	expectedSubject := `Hewlett Packard Conference call on Wireless and Handheld Technologies, December 14th, 1:30-2:30 PM`
	...
	Subject, ok := (*em)["Subject"].(string)
	if !ok {
		t.Fatalf("Error casting EmailI into StdEmail struct")
	}

	if strings.Compare(Subject, expectedSubject) != 0 {
		t.Fatalf("got %q, expected %q", Subject, expectedSubject)
	}
}
